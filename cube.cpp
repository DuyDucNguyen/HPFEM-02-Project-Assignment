...
//Cylinder
constexpr real cyr = 0.5 - bmarg;

constexpr real cyxmin = -1.0;
constexpr real cyxmax = 1.0;

constexpr real cyymin = -1.0;
constexpr real cyymax = 1.0;

constexpr real cyzmin = -1.0;
constexpr real cyzmax = 1.0;
...
class Body : public SubDomain
{
  bool inside(const real* x, bool on_boundary) const
  {
    return on_boundary and
		...
		(x[0] - xbody)*(x[0] - xbody) + (x[1] - ybody)*(x[1] - ybody) + (x[2] - zbody)*(x[2] - zbody) < 4.0;
